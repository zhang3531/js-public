...
;(function($,window,document,undefined){
	...
	var Pagination = function(element,options){
		...
		//判断是否有页头分页
		var $obj_top = null;
		if(opts.topPagination != null && opts.topPagination !=''){
			$obj_top = $(opts.topPagination);
		}
		/**
		 * 设置总页数
		 * @param int page 页码
		...
		this.filling = function(index){
			var html = '';
			var total = opts.totalData;
			html +="共计："+'<span>'+total+'</span>';
		 ...
		 * @param int index 页码
		 */
		this.filling_top = function(index){
			var html = '';
			current = index || opts.current;//当前页码
			var pageCount = this.getTotalPage();
			html +='<span>'+current+'/'+pageCount+'页</span>';
			
			//上一页
			if(current == 1){
				 html += '<b href="javascript:;" class="'+opts.prevCls+'">'+opts.prevContent+'</b>';
			}else{
				html += '<a href="javascript:;" class="'+opts.prevCls+'">'+opts.prevContent+'</a>';
			}
            //当前页
			html += '<span class="'+opts.activeCls+'">'+ current +'</span>';
			//下一页
			if(current == pageCount){
				html += '<b href="javascript:;" class="'+opts.nextCls+'">'+opts.nextContent+'</b>'
			}else{
				html += '<a href="javascript:;" class="'+opts.nextCls+'">'+opts.nextContent+'</a>'
			}
			
			if(opts.topPagination != null && opts.topPagination !=''){
				$obj_top.empty().html(html);
			}
		};
		...
		this.eventBind = function(){
			...
			$obj.off().on('click','a',function(){
				...
				self.filling(index);
				if(opts.topPagination != null && opts.topPagination !=''){
					self.filling_top(index);
				}
				typeof opts.callback === 'function' && opts.callback(self);
			});
			if(opts.topPagination != null && opts.topPagination !=''){
				$obj_top.off().on('click','a',function(){
					if($(this).hasClass(opts.nextCls)){
						var index = parseInt($obj.find('.'+opts.activeCls).text()) + 1;
					}else if($(this).hasClass(opts.prevCls)){
						var index = parseInt($obj.find('.'+opts.activeCls).text()) - 1;
